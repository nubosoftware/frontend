    ...
    async.whilst(
        ...
        function(err) {
            ...
            if (self._lockAquired) {
                self._logger.info("acquire: lock on \'" + self._key + "\' acquired");
                ...
                var errMsg = "acquire: couldn't acquire lock on \'" + self._key + "\'";
                self._logger.error(errMsg);
                callback(null, 0);
            }
        }
...
Lock.prototype.release = function(callback) {
    ...
    Common.redisClient.DEL(self._key, function(err, reply) {
        ...
        if (reply == 1) {
            ...
            self._logger.info("release: lock on \'" + self._key + "\' released");
